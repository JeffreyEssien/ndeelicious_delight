...
export async function POST(request:Request){
  try{
    ...
    const {data:customer,error:customerError}=await service.from("customers").upsert({email:customerEmail,name:parsed.data.customerName,phone:parsed.data.phone},{onConflict:"email"}).select("id").single();
    if(customerError)throw customerError;const requestNumber=`CC-${Date.now().toString().slice(-7)}`;
    const {error}=await service.from("custom_cake_orders").insert({request_number:requestNumber,customer_id:customer.id,customer_name:parsed.data.customerName,email:customerEmail,phone:parsed.data.phone,configuration:parsed.data,requested_date:parsed.data.deliveryDate,estimated_total:quote.estimatedTotal,status:quote.quoteRequired?"QUOTE_REQUIRED":"DRAFT",customer_note:parsed.data.customerNote});
