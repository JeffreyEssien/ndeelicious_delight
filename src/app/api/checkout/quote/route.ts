...
export async function POST(request:Request){
  ...
  try{
    ...
    const {cart,delivery,couponCode}=parsed.data;const [products,deliveryZones,business]=await Promise.all([getProducts(),getDeliveryZones(),getBusinessSettings()]);
    ...
    const quote=calculateOrderQuote({cart,products,fulfilment:delivery.fulfilment,deliveryFee:zone?.fee,deliveryMinimum:zone?.minimumOrder,orderMinimum:business.orderMinimum,deliveryEnabled:business.deliveryEnabled,pickupEnabled:business.pickupEnabled,coupon});
