...
import { emailFrame, sendTransactionalEmail } from "@/lib/email/resend";
...
export async function POST(request: Request) {
  ...
  const { error } = await createServiceClient().from("newsletter_subscribers").upsert(
    { email: parsed.data.email, active: true, unsubscribed_at: null }, { onConflict: "email" },
  );
  ...
  await sendTransactionalEmail({
    ...
    html: emailFrame("You’re on the list", "<p>We’ll share fresh bakes, seasonal boxes and special dates with you.</p>"),
  });
