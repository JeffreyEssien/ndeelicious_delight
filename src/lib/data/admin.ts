...
export async function getAdminData(supabase: SupabaseClient) {
  const [products, zones, orderResult, cakeResult, couponResult, reviewResult] = await Promise.all([
    getProducts({ includeInactive: true, fallback: false, client: supabase }),
    ...
    supabase
      ...
      .select(
        "id,request_number,customer_name,email,phone,status,configuration,requested_date,estimated_total,quoted_total,customer_note",
      )
      .order("created_at", { ascending: false })
      .limit(100),
    supabase
      .from("coupons")
      .select("id,code,type,value,minimum_order,maximum_discount,usage_limit,active,expires_at")
      .order("created_at", { ascending: false }),
    supabase
      .from("reviews")
  ...
  const cakes: AdminCakeRequest[] = (cakeResult.data ?? []).map((row) => ({
    id: row.id,
